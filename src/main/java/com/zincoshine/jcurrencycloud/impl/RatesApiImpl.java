...
public class RatesApiImpl implements RatesApi{
	...
	public Rate findDetailedRates(String buyCurrency, String sellCurrency,
			String fixedSide, double amount,Date conversionDate,String onBehalfOf) throws JCurrencyCloudException {
		Rate rate = null;
		if(token != null) {
			try {
				HttpClient httpClient = new DefaultHttpClient();
				StringBuffer queryString = new StringBuffer();
				queryString.append("buy_currency=").append(URLEncoder.encode(buyCurrency,"UTF-8"));
				queryString.append("&sell_currency=").append(URLEncoder.encode(sellCurrency,"UTF-8"));
				queryString.append("&fixed_side=").append(URLEncoder.encode(fixedSide,"UTF-8"));
				queryString.append("&amount=").append(URLEncoder.encode(amount+"","UTF-8"));
				if(conversionDate != null) {
					queryString.append("&conversion_date=").append(URLEncoder.encode(new DateTime(conversionDate).toString("YYYY-MM-DD"),"UTF-8"));
				}
				if(onBehalfOf != null) {
					queryString.append("&on_behalf_of=").append(URLEncoder.encode(onBehalfOf,"UTF-8"));
				}
				
				HttpGet httpGet = new HttpGet(token.getUrl() + "rates/detailed?" + queryString.toString());
	...
	public Map<String,BidAskRate> findRates(List<String> currencyPair)
		...
		if(token != null) {
			try {
				HttpClient httpClient = new DefaultHttpClient();
				String queryString = StringUtils.join(currencyPair).trim().replaceAll("[\\[\\]\\s]", "");
				
				HttpGet httpGet = new HttpGet(token.getUrl() + "rates/find?currency_pair=" + URLEncoder.encode(queryString,"UTF-8"));
				...
				if (response.getStatusLine().getStatusCode() == 200) {
					...
					JsonObject obj = new Gson().fromJson(jsonOutput,JsonObject.class);
					...
					while(iterator.hasNext()) {
						...
						if(keyValuePair.length == 2) {
							...
							List<String> bidAskRates = Arrays.asList(keyValuePair[1].replaceAll("[\\[\\]\\s]", "").replaceAll("\"","").split(","));
