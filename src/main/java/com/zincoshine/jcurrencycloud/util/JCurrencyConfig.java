...
public class JCurrencyConfig {
	...
	public static void main(String []a) throws Exception{
		...
		AuthToken authToken = authenticate.beginSession(theConfig.getDemoUrl(), theConfig.getLoginId(), theConfig.getApiKey());
		
		System.out.println(authToken);
		/*
		AccountsApi accounts = new AccountsApiImpl();
		AccountVO theAccount = new AccountVO();
		theAccount.setAccountName("Test Account");
		...
		theAccount.setCity("Test");
		theAccount.setCountry("GB");
		theAccount.setPostalCode("test");
		theAccount.setStateOrProvince("test");
		theAccount.setStreet("test");
		theAccount.setSpreadTable("test");
		theAccount.setStatus("Active");
		theAccount.setYourRef("my-ref-1");
		Account newAccount = accounts.createAccount(theConfig.getDemoUrl(), authToken.getToken(), theAccount);
		...
		*/
		...
		//System.out.println(rates.findDetailedRates("USD", "EUR", "buy", 100, null, null));
